...
impl ProfileModRepository {
    ...
    pub fn add_mod_to_profile(&self, profile_mod: &ProfileMod) -> Result<(), String> {
        ...
        let exists = self.connection
            ...
            .query_row(params![profile_mod.profile_id, profile_mod.mod_id], |row| row.get::<_, i32>(0))
            .map_err(|e| format!("检查关联存在性失败: {}", e))? > 0;
    ...
    /// 更新Mod的符号链接路径
    pub fn update_link_path(&self, profile_id: i32, mod_id: i32, link_path: Option<String>) -> Result<(), String> {
        let result = self.connection
            .execute(
        ...
        let profile_mods: Vec<ProfileMod> = stmt
            .query_map(params![profile_id], |row| {
                Ok(ProfileMod {
                    ...
                    link_path: row.get::<_, Option<String>>("link_path")?.map(PathBuf::from),
                })
            })
